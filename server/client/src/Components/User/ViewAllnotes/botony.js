...
export default function Botony() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/viewallnotes")
      ...
      .then((data) => {
        // Filter notes with "accept" status and "botony" branch
        const viewallnotes = data.filter(
          (note) => note.status === "accept" && note.branch === "Botany"
        );
    ...
    <>
      ...
      <div className="viewall-notes m-t">
        <div className="lrgcontainer">
          <h2 className="content-heading">Botony Notes</h2>
